...
//Rare exceptions to indicate a bug in the _wielgosz_michael_project3_localstorage code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_wielgosz_michael_project3_localstorage = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_wielgosz_michael_project3_localstorage]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.wielgosz_michael_project3_localstorage.module"
