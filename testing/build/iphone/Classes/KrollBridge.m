...
NSString * Wielgosz_Michael_Project3_LocalStorage$ModuleRequireFormat = @"(function(exports){"
...
@implementation Wielgosz_Michael_Project3_LocalStorageObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_wielgosz_michael_project3_localstorage);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_wielgosz_michael_project3_localstorage gc];
}
...
{
	// create Wielgosz_Michael_Project3_LocalStorage global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Wielgosz_Michael_Project3_LocalStorage" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_wielgosz_michael_project3_localstorage = [[Wielgosz_Michael_Project3_LocalStorageObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_wielgosz_michael_project3_localstorage];
	
	NSString *_wielgosz_michael_project3_localstorageNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _wielgosz_michael_project3_localstorageNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_wielgosz_michael_project3_localstorage valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Wielgosz_Michael_Project3_LocalStorage$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.wielgosz_michael_project3_localstorage.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.wielgosz_michael_project3_localstorage.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
